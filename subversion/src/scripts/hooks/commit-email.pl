...
# Hacked by Eelco Dolstra <eelco@cs.uu.nl>.
...
# $LastChangedDate: 2002-10-04 23:19:56 +0200 (Fri, 04 Oct 2002) $
...
my @svnlooklines = &read_from_process($svnlook, 'info', $repos, '--revision', $rev);
...
my @dirschanged = &read_from_process($svnlook, 'dirs-changed', $repos, '--revision', $rev);
...
@svnlooklines = &read_from_process($svnlook, 'changed', $repos, '--revision', $rev);
...
my @difflines = &read_from_process($svnlook, 'diff', $repos, '--revision', $rev);
...
push(@body, "\n");
push(@body, "You can view the changes in this commit at:\n");
push(@body, "   @canonicalName@/viewvc/$repo?rev=$rev&view=rev\n");
push(@body, "\n");
if (@adds)
  {
    @adds = sort @adds;
...
foreach my $project (@project_settings_list)
  {
    ...
    if ($commondir ne '')
      {
        $subject = "$subject - in $commondir: $dirlist";
      }
    else
      {
        $subject = "$subject - $dirlist";
      }
    if ($subject_prefix =~ /\w/)
      {
...
#    my $mail_sender = $author;
    ...
    if ($hostname =~ /\w/)
      {
        $mail_sender = "$mail_sender\@$hostname";
      }

    my $userlist = getDB("@dbDir@/svn-watchers", $repo);

    unless (defined($userlist) && $userlist =~ /\w/) {
	print "No watchers for $repo.\n";
	exit;
    };

    my $authorAddress = getDB("@dbDir@/svn-contact", $author);
    my $authorFullName = getDB("@dbDir@/svn-fullnames", $author);

    my $fromAddress = $mail_sender;
    my $replyToAddress;
    if (defined $authorAddress) {
