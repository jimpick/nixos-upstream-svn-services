...
xsltproc -o $out/index.xhtml main-index.xsl $src 
xsltproc --stringparam out "$out" full-indices-per-package.xsl $src

xsltproc -o $out/quick-view.xhtml quick-view.xsl $src 
xsltproc --stringparam sortByDate 1 -o $out/quick-view-by-date.xhtml quick-view.xsl $src 
